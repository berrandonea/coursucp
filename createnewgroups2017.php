...
foreach ($vets as $vet) {
    ...
    foreach ($vetcourses as $vetcourse) {
        ...
        $availinufr[$ufrcodeyear]++;

        //Si ce cours est présent dans la base de données (il a été créé)
        $course = $DB->get_record('course', array('shortname' => "$idvetyear-$elementpedagogique"));

        if ($course) {
            echo "Le cours $idvetyear-$elementpedagogique existe dans Moodle\n";
            $courseid = $course->id;
            ...
            foreach ($vetcoursegroups as $vetcoursegroup) {
                ...
                $codegroupeyear = "Y2017-$codegroupe";
                $group = $DB->get_record('groups', array('codegroupe' => $codegroupeyear, 'courseid' => $courseid));
                if (!$group) { //Si ce groupe n'existe pas encore dans la base de données
                    //On le crée
                    $groupname = $vetcoursegroup->getAttribute('GroupName');
                    $groupid = $DB->insert_record('groups',array('courseid'=>$courseid,'name'=>$groupname,'timecreated'=>time(),'codegroupe'=>$codegroupeyear));
                } else {
                    $groupid = $group->id;
                }
...
foreach ($availinufr as $ufrcodeyear =>$nbcourses) {
    $sql = "UPDATE mdl_ufr SET avail_courses = $nbcourses WHERE code = '$ufrcodeyear'";
    echo "$sql\n";
    $DB->execute($sql);
}

foreach ($nbvetsinufr as $ufrcodeyear =>$nbvets) {
    $sql = "UPDATE mdl_ufr SET promotions = $nbvets WHERE code = '$ufrcodeyear'";
    echo "$sql\n";
    $DB->execute($sql);
}
