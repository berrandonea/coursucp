        ...
        foreach ($questions as $question) {
            ...
            if (!empty($answers)) {
                ...
                foreach ($answers as $answer) {
                    if ($answer->correct == 1) {
			//BRICE
			$answertextbrice = format_text(s($answer->answertext), $answer->answertextformat);
                        echo '<td width="10%" class="realtimequiz_report_question_correct"><b>'.$answertextbrice.'</b></td>';
			//FIN
//BRICE                 echo '<td width="10%" class="realtimequiz_report_question_correct"><b>'.s($answer->answertext).'</b></td>';
                        $iscorrectanswer = true;
                    } else {
			//BRICE
			$answertextbrice = format_text(s($answer->answertext), $answer->answertextformat);
                        echo '<td width="10%">'.$answertextbrice.'</td>';
			//FIN
//BRICE                 echo '<td width="10%">'.s($answer->answertext).'</td>';
                    }
                }
...
//BRICE                echo '</tr><tr class="realtimequiz_report_answer"><td><a href="'.$linkurl->out(true, array('questionid'=>$question->id)).'">'.format_string($questiontext).'</a></td>';
                echo '</tr><tr class="realtimequiz_report_answer"><td><a href="'.$linkurl->out(true, array('questionid'=>$question->id)).'">'.format_text($questiontext, $question->questiontextformat).'</a></td>'; //BRICE
    ...
    if (!empty($answers)) {
        foreach ($answers as $answer) {
	    $answertextbrice = format_text(s($answer->answertext), $answer->answertextformat); //BRICE
            if ($answer->correct == 1) {
                echo '<td width="10%"><b>'.$answertextbrice.'</b></td>'; //BRICE
//BRICE                echo '<td width="10%"><b>'.s($answer->answertext).'</b></td>';
            } else {
                echo '<td width="10%">'.$answertextbrice.'</td>'; //BRICE
//BRICE                echo '<td width="10%">'.s($answer->answertext).'</td>';
            }
        }
    }
