...
//BRICE                $shortname = format_string($course->shortname, true, array('context' => context_course::instance($course->id)));
                $shortname = format_string($course->fullname, true, array('context' => context_course::instance($course->id))); //BRICE
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    //echo "$post->userid<br>$post->created<br>$post->modified<br>";     
    $sql = "SELECT firstname, lastname FROM mdl_user WHERE id = $post->userid";
    $postauthor = $DB->get_record_sql($sql);
    echo "Ecrit par <a href='https://cours.u-cergy.fr/user/view.php?id=$post->userid&course=$COURSE->id'>$postauthor->firstname $postauthor->lastname</a>";
    $writtendate = date("d/m/Y H:i:s", $post->created);
    echo " le $writtendate";

    if ($post->modified != $post->created) {
        $modifieddate = date("d/m/Y H:i:s", $post->modified);
        echo " (modifié le $modifieddate)";
    }
    echo " :";
    //FIN    
