...
/*        //BRICE
        echo "<br><span style='color:red;margin-left:10px;font-weight:bold;text-align:center'>
        		ATTENTION: Votre webconférence ne sera accessible qu'après avoir reçu l'accord du SEFIAP.
        		Une demande sera automatiquement envoyée au SEFIAP quand vous aurez validé ce formulaire.</span><br>";
        echo "<span style='color:red;margin-left:10px;font-weight:bold;text-align:center'>
        		L'objectif est d'éviter de surcharger le serveur avec plusieurs webconférences simultanées.
        		Si nécessaire, nous prendrons contact avec vous pour convenir d'une autre date.</span><br><br>";
        //FIN*/
...
/*/BRICE*/        if( isset($current_activity->openingtime) && $current_activity->openingtime != 0 || isset($current_activity->closingtime) && $current_activity->closingtime != 0 )
            $mform->setExpanded('schedule');

/*/BRICE*/        $mform->addElement('date_time_selector', 'openingtime', get_string('mod_form_field_openingtime', 'bigbluebuttonbn'), array('optional' => true));
        //$mform->addElement('date_time_selector', 'openingtime', get_string('mod_form_field_openingtime', 'bigbluebuttonbn'), array('optional' => false)); //BRICE
        $mform->setDefault('openingtime', 0);
/*/BRICE*/        $mform->addElement('date_time_selector', 'closingtime', get_string('mod_form_field_closingtime', 'bigbluebuttonbn'), array('optional' => true));
//        $mform->addElement('date_time_selector', 'closingtime', get_string('mod_form_field_closingtime', 'bigbluebuttonbn'), array('optional' => false)); //BRICE
  ...
  /*      //BRICE
    	...
    	$usernames = "$USER->firstname $USER->lastname ($USER->email)";
    	$coursenames = "($COURSE->idnumber) $COURSE->fullname";
        $current_activity =& $this->current;
        //FIN*/
	...
	if ($data['openingtime'] == 0) {
		$errors['openingtime'] = "Vous n'avez pas indiqué de date/heure de début.";
	}
	if ($data['closingtime'] == 0) {
		$errors['closingtime'] = "Vous n'avez pas indiqué de date/heure de fin.";
	}

        if ($data['closingtime'] - $data['openingtime'] > 5 * 3600) {
        	$errors['closingtime'] = "Une webconférence ne peut pas durer plus de 5 heures.";
        }

        $to  = $USER->email.
        ',cosette.abi-dib@u-cergy.fr,hoang-yen.tran-kiem@u-cergy.fr,Thierry.Marchand@iufm.u-cergy.fr,caroline.pers@u-cergy.fr';

        // Sujet
        $subject = 'Demande de webconférence - Plateforme Pedagogique';

        // Message
        $message = "
        <html>
        <head>
        <title>Demande de webconférence - Plateforme Pedagogique</title>
        </head>
        <body>
        <p>Une demande de webconférence vient d'être déposée sur la plateforme pédagogique.</p>
        Par: $usernames<br><br>
        Cours: $coursenames<br><br>
        Du : ".date("Y-m-d H:i:s", $data['timeavailable'])."<br/><br/>
        Au : ".date("Y-m-d H:i:s", $data['timedue'])."</br></br>
        CoursUCP, votre plateforme pédagogique<br>
	Ceci est un message automatique. Merci de ne pas y répondre. 
	Pour toute demande ou information, nous vous invitons à 
	<a href='https://monucp.u-cergy.fr/uPortal/f/u312l1s6/p/Assistance.u312l1n252/max/render.uP?pCp'>Effectuer une demande</a>
	 dans la catégorie <strong>SEFIAP -> Applications pédagogiques</strong>.
        ...
        $headers .= 'Reply-To: noreply@cours.u-cergy.fr'."\r\n".'Content-type: text/html; charset=utf-8'."\r\n".
		    'X-Mailer: PHP/'.phpversion();

        // En-têtes additionnels
        $headers .= 'Bcc: brice.errandonea@u-cergy.fr,noa.randriamalaka@u-cergy.fr' . "\r\n";
