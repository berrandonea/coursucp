    ...
    protected function calculate_risks(array $userids) {
        ...
        foreach ($userids as $userid) {
            ...
            if (!isset($this->rawdata->posts[$userid])) {
                ...
                $reason->logic = "Cet utilisateur n'a jamais fait un post ou avait suivi les messages dans le cours, donc il a un risque maximal de 100%.";
            ...
            $reason->logic = "0% de risuqe pour plus de {$this->config['no_replies']} réponses par semaine. ".
                             "100% pour {$this->config['max_replies']} réponses par semaine.";
            $reason->riskcontribution = intval($risk_contribution*100).'%';
          ...
          /*  $reason->logic = "0% risk for more than {$this->config['no_newposts']} replies a week. ".
            ...
            $reason->logic = "0% de risuqe pour plus de {$this->config['no_newposts']} réponses par semaine. ".
                             "100% pour {$this->config['max_newposts']} nouveaux messages par semaine.";
            $reason->riskcontribution = intval($risk_contribution*100).'%';
           ...
           /* $reason->logic = "0% risk for more than {$this->config['no_readposts']} read posts a week. ".
          	...
          	$reason->logic = "0% de risuqe pour plus de {$this->config['no_readposts']} lecture des messages par semaine. ".
                             "100% pour {$this->config['max_readposts']} lecture des messages par semaine.";
