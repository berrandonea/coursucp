...
function chat_format_message_theme ($message, $chatuser, $currentuser, $groupingid, $theme = 'bubble') {
    ...
    //BRICE : l'auteur du message est-il enseignant dans ce cours ?
    $sql = "SELECT id FROM mdl_context WHERE contextlevel = 50 AND instanceid = $courseid";
    $coursecontextid = $DB->get_record_sql($sql)->id;
    $sql = "SELECT roleid FROM mdl_role_assignments WHERE contextid = $coursecontextid AND userid = $sender->id";
    $senderroles = $DB->get_recordset_sql($sql);
    $senderteacher = false;
    foreach ($senderroles as $senderrole) {
        if (($senderrole->roleid == 3)||($senderrole->roleid == 4)) {
            $senderteacher = true;
        }
    }
    //FIN
