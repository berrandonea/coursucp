...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->data = array();
        $behaviour = $attemptobj->get_quiz()->preferredbehaviour; //BRICE
        ...
        foreach ($slots as $slot) {
            ...
            if ($attemptobj->can_navigate_to($slot)) {                
                ...
                //BRICE
                if ($behaviour == 'ucpcbm') {
                    $questionname = $attemptobj->get_question_name($slot);
                    if (substr($questionname, 0, 16) == 'Auto-évaluation') {
                        $row = array(html_writer::link($attemptobj->attempt_url($slot),
                        $attemptobj->get_question_number($slot) . $flag), '');                       
                    }
                }
...
//                        $savedstatus = get_string('answersaved', 'question');
//                        if ($questionstatus != $savedstatus) {
//                            $canfinishattempt = false;
//                            break;
//                        }
//                    }                    
//                }
...
//        //FIN
      ...
      //  if ($canfinishattempt) { //BRICE            
        ...
        /*//BRICE
            ...
            echo 'Vous devez répondre à toutes les questions. Si vous ne savez pas quoi répondre, répondez au hasard et cochez le degré de certitude "Aucune idée".';
            echo '</p>';
        }
        //FIN*/
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        ...
        if ($quiz->preferredbehaviour == 'ucpcbm') {
            ...
            //$output .= '<p style="text-align:center">Pour chaque question, un niveau de certitude vous sera demandé.';
            //$output .= ' Plus vous affirmez être certain de votre réponse, plus vous marquez de points si elle est bonne et plus vous en perdez si elle est fausse.</p>';
            //$output .= get_string('certainty_explain', 'qbehaviour_ucpcbm').'<br><br>';
            ?>
            <style>
            ...
            <?php
            /*
            $output .= '<table><tr><th class="qbehaviour_ucpcbm_columnheader">'.get_string('certainty_level', 'qbehaviour_ucpcbm').'</th>';
            $levels = array(-1, 1, 2, 3);
            foreach($levels as $level) {
                $output .= "<th class='qbehaviour_ucpcbm_columnheader'>".get_string("certainty$level", 'qbehaviour_ucpcbm')."</th>";
            }
            $output .= '</tr><tr><td class="qbehaviour_ucpcbm_cell">'.get_string('mark_if_correct', 'qbehaviour_ucpcbm').'</td>';
            $scales = explode('.', get_string('mark_values', 'qbehaviour_ucpcbm'));
            $goodmarks = explode(',', $scales[0]);
            $badmarks = explode(',', $scales[1]);
            foreach ($goodmarks as $goodmark) {
                $output .= "<td class='qbehaviour_ucpcbm_cell'>$goodmark</td>";
            }
            $output .= '</tr><tr><td class="qbehaviour_ucpcbm_cell">'.get_string('mark_if_wrong', 'qbehaviour_ucpcbm').'</td>';
            foreach ($badmarks as $badmark) {
                $output .= "<td class='qbehaviour_ucpcbm_cell'>$badmark</td>";
            }
            $output .= '</tr>';
            $output .= '</table><br>';
            //$output .= '<pre>'.get_string('certainty_help', 'qbehaviour_ucpcbm').'</pre>';
            $output .= '<p style="font-weight:bold;color:red;text-align:center">';
            $output .= 'Vous devez répondre à toutes les questions. Si vous ne savez pas quoi répondre, répondez au hasard et cochez le degré de certitude "Aucune idée".';
            $output .= '</p>';
             * 
             */
            $studentreporturl = "$CFG->wwwroot/question/behaviour/ucpcbm/studentreport.php?id=$cm->id";
            $teacherreporturl = "$CFG->wwwroot/question/behaviour/ucpcbm/teacherreport.php?id=$cm->id";
            $output .= "<p style='text-align:center'><a href='$studentreporturl'>";
            $output .= "<button>Rapport pour l'étudiant(e)</button>";
            if (has_capability('mod/quiz:grade', $context)) {
                $output .= "</a>&nbsp;&nbsp;<a href='$teacherreporturl'><button>Rapport pour l'enseignant(e)</button>";
            }
            $output .= '</a></p>';            
        } else {
            //FIN
