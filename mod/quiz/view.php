...
// BRICE.
// Check outcomes questions if ucpcbm.
if ($quiz->preferredbehaviour == 'ucpcbm') {
    $qtype = 'numerical';
    ...
    foreach($quizslots as $quizslot) {
        if ($quizslot->slot > $lastusedslot) {
            $lastusedslot = $quizslot->slot;
        }
        if ($quizslot->page > $lastusedpage) {
            $lastusedpage = $quizslot->page;
        }
        $slotquestion = $DB->get_record('question', array('id' => $quizslot->questionid));
        $questionnames[] = $slotquestion->name;
    }
    $outcomeslot = $lastusedslot + 1;
    $outcomepage = $lastusedpage + 1;
    $params = array('itemtype' => 'mod', 'itemmodule' => 'quiz', 'iteminstance' => $quiz->id);
    $gradeitems = $DB->get_records('grade_items', $params);
    //Pour chaque objectif utilisé dans le quiz, on crée une question d'auto-évaluation et une section
    foreach ($gradeitems as $gradeitem) {
        if ($gradeitem->outcomeid) {
            $outcome = $DB->get_record('grade_outcomes', array('id' => $gradeitem->outcomeid));
            $evalquestionname = "Auto-évaluation $outcome->shortname";
            if (!in_array($evalquestionname, $questionnames)) {                
                //Création de la question si elle n'existe pas encore
                $outcomequestion = $DB->get_record('question', array('name' => $evalquestionname));
                if (!$outcomequestion) {
                    $outcomequestion = new stdClass();
                    $outcomequestion->category = 100;
                    $outcomequestion->name = $evalquestionname;
                    $autoevalquestiontext = "Estimez, par un nombre entre 0 et 10, votre maîtrise de ce pré-requis : $outcome->fullname."; //get_string('autoevalquestiontext', 'qbehaviour_ucpcbm');
                    $outcomequestion->questiontext = $autoevalquestiontext;
                    $outcomequestion->questiontextformat = 1;
                    $outcomequestion->defaultmark = 0.0000000;
                    $outcomequestion->penalty = 0.0000000;
                    $outcomequestion->qtype = 'numerical';
                    $outcomequestion->generalfeedback = '';
                    $outcomequestion->generalfeedbackformat = 1;
                    $now = time();
                    $outcomequestion->timecreated = $now;
                    $outcomequestion->timemodified = $now;
                    $outcomequestion->createdby = $USER->id;
                    $outcomequestion->modifiedby = $USER->id;
                    $outcomequestion->id = $DB->insert_record('question', $outcomequestion);
                    //La réponse
                    $outcomeanswer = new stdClass();
                    $outcomeanswer->question = $outcomequestion->id;
                    $outcomeanswer->answer = 5;
                    $outcomeanswer->answerformat = 1;
                    $outcomeanswer->fraction = '1.0000000';
                    $outcomeanswer->feedback = '';
                    $outcomeanswer->feedbackformat = 1;
                    $outcomeanswer->id = $DB->insert_record('question_answers', $outcomeanswer);
                    //Options dans question_numerical et question_numerical_options
                    $outcomeqn = new stdClass();
                    $outcomeqn->question = $outcomequestion->id;
                    $outcomeqn->answer = $outcomeanswer->id;
                    $outcomeqn->tolerance = 5;
                    $outcomeqn->id = $DB->insert_record('question_numerical', $outcomeqn);
                    $outcomeqno = new stdClass();
                    $outcomeqno->question = $outcomequestion->id;
                    $outcomeqno->unitpenalty = 0;
                    $outcomeqno->id = $DB->insert_record('question_numerical_options', $outcomeqno);                    
                }
                //Ajout de la question dans le quiz
                quiz_add_quiz_question($outcomequestion->id, $quiz, 0, 0);
//                $quizslot = new stdClass();
//                $quizslot->slot = $outcomeslot;
//                $outcomeslot++;
//                $quizslot->quizid = $quiz->id;
//                $quiz->page = $outcomepage;
//                $quiz->questionid = $outcomequestion->id;                    
                    //$question = $qtypeobj->save_question($question, $fromform);
                //TODO : Créer le gradeitem sur le slot ($lastusedslot+1)                
            }
        }
    }
}
...
//    [courseid] => 2
//    [categoryid] => 1
//    [itemname] => Maîtriser le calcul algébrique littéral
//    [itemtype] => mod
//    [itemmodule] => quiz
//    [iteminstance] => 4
//    [itemnumber] => 1000
...
//    [needsupdate] => 0
//    [weightoverride] => 0
//    [timecreated] => 1464186276
//    [timemodified] => 1464186276
//)
//
//stdClass Object
//(
...
//    [courseid] => 
//    [shortname] => Maths1
//    [fullname] => Maîtriser le calcul algébrique littéral
//    [scaleid] => 2
//    [description] => 
//    [descriptionformat] => 0
//    [timecreated] => 1464171318
//    [timemodified] => 1464171318
//    [usermodified] => 6
//)
