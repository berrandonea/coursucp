...
$sql = "SELECT * FROM `mdl_course_categories` WHERE parent = 395 AND LENGTH(idnumber) = 7 AND idnumber <> 'Y2017-6'";
$composantes = $DB->get_records_sql($sql);

foreach ($composantes as $composante) {
	$sql = "SELECT count(distinct vets.id) as nbrvets FROM `mdl_cat_demandecours` ufr, `mdl_cat_demandecours` niveau, mdl_cat_demandecours vets where niveau.id = vets.parent_id and ufr.id = niveau.parent_id AND ufr.code = '".substr($composante->idnumber, -1)."'";
  ...
  $graph = new PieGraph(650,550,"pieex1");
  $graph->SetShadow();
