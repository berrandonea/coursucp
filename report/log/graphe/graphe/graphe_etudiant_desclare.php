...
foreach ($composantes as $composante) {
	$sql = "SELECT COUNT(userid) AS nb FROM mdl_ufr_student WHERE ufrcode = '".$composante->idnumber."' AND student = 1";
    $nbknownstudentsincomposante = $DB->get_record_sql($sql)->nb;
    if (!$nbknownstudentsincomposante) {
		$nbknownstudentsincomposante = 1;
	}
    //~ $composante["nbknownstudentsincomposante"] = $nbknownstudentsincomposante;	
	$tableauNombreVentes = array_merge($tableauNombreVentes ,array("$nbknownstudentsincomposante"));
  ...
  $graph->title->Set("Étudiants déclarés");
