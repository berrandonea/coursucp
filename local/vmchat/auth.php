...
//BRICE : limiter à l'EAD ESPE.
$vmchater = false;
$espeeadcategory = $DB->get_record('course_categories', array('idnumber' => 'A-EAD2')); // A adapter lors d'un changement de plateforme.
$espeeadcontext = context_coursecat::instance($espeeadcategory->id);
$espeeadmanager = has_capability('moodle/course:create', $espeeadcontext);
if ($espeeadmanager) {
    $vmchater = true;
}
$sql = "SELECT * FROM {context} WHERE path LIKE '$espeeadcontext->path/%' AND contextlevel = ".CONTEXT_COURSE;
$espeeadcoursecontexts = $DB->get_recordset_sql($sql);
foreach ($espeeadcoursecontexts as $coursecontext) {
    $isenroled = $DB->record_exists('role_assignments', array('contextid' => $coursecontext->id, 'userid' => $USER->id));
    if ($isenroled) {
        $vmchater = true;
    }
}
$espeeadcoursecontexts->close();
if (!$vmchater) {
    exit;
}
//FIN*/
