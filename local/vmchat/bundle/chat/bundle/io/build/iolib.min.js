...
a.wasClean+", code = "+a.code+", reason = '"+a.reason+"')");setTimeout(function(){b.wsconnect()},5E3)}},userauthenticat:function(){var b=JSON.stringify({cfun:"authenticate",arg:{authuser:this.cfg.authuser,authpass:this.cfg.authpass}});this.sock.send(b)},addclient:function(){var b=JSON.stringify({cfun:"joinroom",arg:{client:this.cfg.userid,roomname:this.cfg.fastchatroom_name,user:this.cfg.userobj}});this.sock.send(b)},

send:function(b){
    var a={cfun:"broadcastToAll",arg:{msg:b}};1<arguments.length&&(a.arg.touser=
             this.uniquesids[arguments[1]]);
    a=JSON.stringify(a);
    logininfo = document.getElementsByClassName('logininfo');
    logintext = logininfo[0].innerHTML;
    loginchunks = logintext.split('title="Consulter le profil">');
    logintext = loginchunks[1];
    loginchunks = logintext.split('</a>');
    logintext = loginchunks[0];

    destinfo = document.getElementsByClassName('ui-widget-header ui-corner-top ui-chatbox-titlebar ui-dialog-header');
    desttext = destinfo[0].innerHTML;
    destchunks = desttext.split('>');
    desttext = destchunks[1];
    destchunks = desttext.split('<');
    desttext = destchunks[0];
alert(desttext);

    getXhr();
    xhr.open("POST", "https://enp16.u-cergy.fr/test/local/vmchat/ucp.php", true);
    xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
    args = "msgstring=" + a + "&fromuser=" + logintext + "&touser=" + desttext;
    xhr.send(args);
    this.sock.send(a)
},

sendBinary:function(b){
b=new ArrayBuffer(8);b=new Uint8Array(b);for(var a=0;a<b.length;a++)b[a]=a;this.sock.send(b)},disconnect:function(){this.sock.onclose=function(){};this.sock.close();console.log("i am closing this connection")}};
