(function(b){
	b.widget(
	"ui.footerbar",{
		options:{id:null,offset:0,width:"100%",barscroll:b(window).width()-430,boxClosed:function(a){}},
		init:function(a){this.elem=a},
		widget:function(){return this.uiFooterbar},
		_create:function(){
			var a=(this.uiFooterbar=b("<div></div>")).appendTo(document.body).prop("id","stickybar").addClass("maximize"),
			c=(this.uiFooterbarClosebox=b("<div></div>")).addClass("vmchat_close_bt").prop("id","hide_bar").appendTo(a);
			(this.uiFooterbarClose=b("<input>")).prop("id","closebtn").prop("type","button").prop("title","Close chat").addClass("close").appendTo(c).clickToggle(
			    function(){
					if(b("#stickybar").hasClass("maximize")){
						b("#stickybar").removeClass("maximize").addClass("minimize");
						b("#hide_bar input").removeClass("close").addClass("expand");
						b("#hide_bar input").prop("title","Open chat");
						io.disconnect();
						b(".ui-memblist").remove();
						b(".ui-chatbox").remove();
						b("div#chatrm").remove();
						chatroombox=null;
						for(key in io.uniquesids) key!=io.cfg.userid&&(chatboxManager.delBox(key),b("li#tabcb"+key).remove());
                        idList=[];
                        tabs.tabs("refresh");
                        localStorage.clear();
                        vmstorage={};
                ...
                function(){
				    b("#stickybar").removeClass("minimize").addClass("maximize");
					b("#hide_bar input").removeClass("expand").addClass("close");
					b("#hide_bar input").prop("title","Close chat");
					localStorage.clear();
					io.init(userdata)
				}
			);
			uiFooterbarchatroomtab=(this.uiFooterbarchatroomtab=b("<div></div>")).addClass("vmchat_room_bt").prop("id","chatroom_bt").appendTo(a);
            uiFooterbarchatroomContent=(this.uiFooterbarchatroomContent=b('<div class = "inner_bt"></div>')).appendTo(uiFooterbarchatroomtab);
            uiFooterbarchatroomIcon=(this.uiFooterbarchatroomIcon=b('<div id = "chatroom_icon"></div>')).appendTo(uiFooterbarchatroomContent);
            uiFooterbarchatroomText=(this.uiFooterbarchatroomText=b('<div id = "chatroom_text"></div>')).appendTo(uiFooterbarchatroomContent).text("Chat collectif").click(
                function(){
				    if(chatroombox)"hidden"==sessionStorage.getItem("chatroom_status")?sessionStorage.removeItem("chatroom_status"):sessionStorage.setItem("chatroom_status","hidden"),chatroombox.chatroom("option","boxManager").toggleBox();
                    else if(0==b("div#chat_room").length){
						var a=document.createElement("div");
						a.id="chat_room";document.body.appendChild(a);
						chatroombox=b("#chat_room").chatroom(
						    {
								id:"chat_room",user:{name:"test"},title:lang.chatroom_header,offset:"20px",
								messageSent:function(a,c){b("#chat_room").chatroom("option","boxManager").addMsg(a.name,c)}
							}
						)
					}
				}
			);
			uiFooterbarUserlisttab=(this.uiFooterbarUserlisttab=b("<div></div>")).addClass("vmchat_bar_button").prop("id","user_list").appendTo(a);
			uiFooterbarUserlistContent=(this.uiFooterbarUserlistContent=b('<div class="inner_bt"></div>')).appendTo(uiFooterbarUserlisttab);
			uiFooterbarUserlistIcon=(this.uiFooterbarUserlistIcon=b('<div id="usertab_icon"></div>')).appendTo(uiFooterbarUserlistContent);
			uiFooterbarUserlistText=(this.uiFooterbarUserlistText=b('<div id="usertab_text"></div>')).appendTo(uiFooterbarUserlistContent).text("Chat privé").click(
			    function(){1<Object.keys(io.uniquesids).length&&b("#chat_div").memberlist("option","boxManager").toggleBox()}
			);
			uiFooterbartabCont=(this.uiFooterbartabCont=b("<div></div>")).attr("id","tabs").addClass("tabs-bottom").appendTo(a);
			uiFooterbartabs=(this.uiFooterbartabs=b('<ul class="tabs"></ul>')).appendTo(uiFooterbartabCont);
			this._setWidth(this.options.width);
			this.init(this)
		},
		destroy:function(){
			this.element.remove();this._destroy()
		},
		_setWidth:function(a){
			this.uiFooterbar.width(a+"px")
		}
	}
	)
})
...
function display_error(b){
    ...
    $("#dialog").html(b);
    $("#dialog").dialog()};
    var lang={online:"Utilisateurs en ligne",whos:"Utilisateurs en ligne",chatroom:"Chat collectif",chatroom_header:"Chat collectif",error:"Something bad happend. Sorry!",wserror:"Browser does not support WebSocket",sterror:"This browser doesn't support Web Storage standard"
};
