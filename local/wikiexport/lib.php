...
class local_wikiexport {
	...
	//BRICE WKS Export/import
	if (has_capability('mod/wiki:managewiki', $context)) {
            $exportname = "Exporter au format WKS";
            $importname = "Importer au format WKS";
            $wkspage = '/local/wikiexport/wksport.php';
            $exporturl = new moodle_url($wkspage, array('action' => 'export', 'cmid' => $cm->id));
            $importurl = new moodle_url($wkspage, array('action' => 'import', 'cmid' => $cm->id));
            $ret[$exportname] = $exporturl;
            $ret[$importname] = $importurl;
        }
	//FIN
        return $ret;
    }
