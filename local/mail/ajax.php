...
function local_mail_getrecipients($message, $search, $groupid, $roleid) {
    ...
    // This is to adapt the list of recipients when we send a message outside of the course system.
	// HACK BEGINNING
    ...
    if ($message->course()->id == 1) {
        ...
        // Get all users who do not have a role in the same context as us and remove them from matchcount.
        ...
        $result3list = implode(', ', $result3);

        if (!empty($search)) {

            $fullname = $DB->sql_fullname('firstname', 'lastname');
            $params2['search1'] = "%$search%";
            $where2 = "WHERE id IN ($result3list) AND (". $DB->sql_like($fullname,
                    ':search1', false, false) .") ";
            $matchcount2 = $DB->count_records_sql("SELECT COUNT(DISTINCT id) FROM {user}"
                    . " $where2", $params2);

            $matchcount = $matchcount - $matchcount2;
        } else {

            $matchcount2 = $DB->count_records_sql("SELECT COUNT(DISTINCT id) FROM {user}"
                    . " WHERE id NOT IN ($result2list) AND id != 1 AND id != 2");
