...
if (!$course = $DB->get_record('course', array('id' => $courseid))) {
    ...
    // This is part of the code to add the 'allcourse' option when creating a message.
	// HACK BEGINNING 
    if ($courseid != 0) {
...
if ($course->id != $SITE->id and has_capability('local/mail:usemail', $context)) {
    ...
    redirect($url);
} else if ($course->id == $SITE->id && $courseid == 0) {
...
$coursescurrentyear = null;
$coursespreviousyear = null;
$coursescollab = null;
$coursesperso = null;

foreach ($courses as $course){

    if (strstr($course->idnumber, "Y2017") == true) {

        $course->fullname = "[2017-2018] ".$course->fullname;
        $coursescurrentyear[] = $course;
    } else if (strstr($course->idnumber, "COLLAB")) {
        
        $course->fullname = "[COLLAB] ".$course->fullname;
        $coursescollab[] = $course;
    } else if (strstr($course->idnumber, "PERSO")) {

        $course->fullname = "[PERSO] ".$course->fullname;
        $coursesperso[] = $course;
    } else {

        $course->fullname = "[2016-2017] ".$course->fullname;
        $coursespreviousyear[] = $course;
    }
}

$orderedcourses = $coursescurrentyear;

foreach ($coursespreviousyear as $coursepreviousyear) {

    $orderedcourses[] = $coursepreviousyear;
}

foreach ($coursescollab as $coursecollab) {

    $orderedcourses[] = $coursecollab;
}

foreach ($coursesperso as $courseperso) {

    $orderedcourses[] = $courseperso;
}

$customdata = array('courses' => $orderedcourses);
...
echo $OUTPUT->header();
local_mail_add_shortcuts('compose');
if ($courses) {
    $mform->display();
} else {
