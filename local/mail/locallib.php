...
function local_mail_format_content($message) {
    ...
    $content = file_rewrite_pluginfile_urls($message->content(), 'pluginfile.php', $context->id,
                                            'local_mail', 'attachmentintext', $message->id());
...
function local_mail_send_notifications($message) {
    ...
    foreach ($message->recipients() as $userto) {
        ...
        if ($message->has_attachment(false)) {
            $plaindata->content .= '/n'. get_string('attachmentonucp', 'local_mail');
            $htmldata->content .= '<p>'. get_string('attachmentonucp', 'local_mail').'</p>';
        }
...
//            if (strstr($course->idnumber, "Y2017") == false || strstr($course->idnumber, "Y2016") == false ) {
//
//                unset($courses[$course->id]);
//            }
        }
...
function local_mail_add_shortcuts($type) {

    global $CFG, $DB, $USER;
    $compose = get_string('compose', 'local_mail');
    $inbox = get_string('inbox', 'local_mail');
    $mark = get_string('starredmail', 'local_mail');
    $draft =get_string('drafts', 'local_mail');
    $sent =get_string('sentmail', 'local_mail');
    $preferences = get_string('preferences', 'local_mail');
    $sqlcountdraft = "SELECT COUNT( id ) AS nbr FROM mdl_local_mail_index WHERE userid =$USER->id AND TYPE ='drafts'";
    $countdraft =$DB->get_record_sql($sqlcountdraft);


    $sqlcountdraft = "SELECT count(distinct lmidraft.id) as nbr
    FROM mdl_local_mail_index lmidraft, mdl_local_mail_index lmicourse, mdl_course c
    WHERE lmidraft.userid =$USER->id AND lmidraft.TYPE ='drafts' and lmicourse.type ='course'
    and lmidraft.messageid = lmicourse.messageid and c.id = lmicourse.item";
    $countdraft =$DB->get_record_sql($sqlcountdraft);

    $sqlunread ="SELECT count(distinct lmidraft.id) as nbr
    FROM mdl_local_mail_index lmidraft, mdl_local_mail_index lmicourse, mdl_course c
    WHERE lmidraft.userid =$USER->id AND lmidraft.TYPE ='inbox' and lmidraft.unread =1 and lmicourse.type ='course'
    and lmidraft.messageid = lmicourse.messageid and c.id = lmicourse.item";
    $countunread = $DB->get_record_sql($sqlunread);

    echo "<table><tr>";

    if ($type =='compose') {

        echo "<td><center><a href = '$CFG->wwwroot/local/mail/create.php'><img src='$CFG->wwwroot/local/mail/images/RedigerPom.png' height='50' width='50'><br><strong>$compose</strong></a></center></td>";
    } else {

        echo "<td><center><a href = '$CFG->wwwroot/local/mail/create.php'><img src='$CFG->wwwroot/local/mail/images/Rediger.png' height='35' width='35'><br>$compose</a></center></td>";
    }

    if($type =='inbox' || $type=='label')
    {
        if($countunread->nbr >=1)
        {
            echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=inbox'><img src='$CFG->wwwroot/local/mail/images/ReceptionPom.png' height='50' width='50'><br><strong>$inbox ($countunread->nbr)</strong></a></center></td>";
        }
        else
        {
            echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=inbox'><img src='$CFG->wwwroot/local/mail/images/ReceptionPom.png' height='50' width='50'><br><strong>$inbox</strong></a></center></td>";
        }
    }
    else
    {
        if($countunread->nbr >=1)
        {
            echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=inbox'><img src='$CFG->wwwroot/local/mail/images/reception.png' height='35' width='35'><br>$inbox ($countunread->nbr)</a></center></td>";
        }
        else
        {
            echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=inbox'><img src='$CFG->wwwroot/local/mail/images/reception.png' height='35' width='35'><br>$inbox</a></center></td>";
        }
    }

    if($type =='sent')
    {
        echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=sent'><img src='$CFG->wwwroot/local/mail/images/sent-mailPom.png' height='50' width='50'><br><strong>$sent</strong></a></center></td>";
    }
    else
    {
        echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=sent'><img src='$CFG->wwwroot/local/mail/images/sent-mail.png' height='35' width='35'><br>$sent</a></center></td>";
    }
    ...
    if($type =='drafts')
    {
        if($countdraft->nbr >=1)
        {
            echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=drafts'><img src='$CFG->wwwroot/local/mail/images/BrouillonPom.png' height='50' width='50'><br><strong>$draft ($countdraft->nbr)</strong></a></center></td>";
        }
        else
        {
            echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=drafts'><img src='$CFG->wwwroot/local/mail/images/BrouillonPom.png' height='50' width='50'><br><strong>$draft</strong></a></center></td>";
        }
    }
    else
    {
        if($countdraft->nbr >=1)
        {
            echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=drafts'><img src='$CFG->wwwroot/local/mail/images/Brouillons.png' height='35' width='35'><br>$draft ($countdraft->nbr)</a></center></td>";
        }
        else
        {
            echo "<td><center><a href ='$CFG->wwwroot/local/mail/view.php?t=drafts'><img src='$CFG->wwwroot/local/mail/images/Brouillons.png' height='35' width='35'><br>$draft</a></center></td>";
        }
    }

    if ($type == 'starred')
    {
        echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=starred'><img src='$CFG->wwwroot/local/mail/images/MarquePom.png' height='50' width='50'><br><strong>$mark</strong></a></center></td>";
    }
    else
    {
        echo "<td><center><a href = '$CFG->wwwroot/local/mail/view.php?t=starred'><img src='$CFG->wwwroot/local/mail/images/marque2.png' height='35' width='35'><br>$mark</a></center></td>";
    }

    if ($type == 'preferences') {
        echo "<td><center><a href = '$CFG->wwwroot/local/mail/preferences.php'><img src='$CFG->wwwroot/local/mail/images/parametresPom.png' height='50' width='50'><br><strong>$preferences</strong></a></center></td>";
    } else {
        echo "<td><center><a href = '$CFG->wwwroot/local/mail/preferences.php'><img src='$CFG->wwwroot/local/mail/images/parametres.png' height='35' width='35'><br>$preferences</a></center></td>";
    }
    echo "</tr></table><br>";
}
