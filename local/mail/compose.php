...
$textareaid = file_get_submitted_draft_itemid('attachmentsintext');
$content = file_prepare_draft_area($textareaid, $PAGE->context->id,
                                   'local_mail', 'attachmentintext', $message->id(),
                                   mail_compose_form::file_options(),
                                    $message->content());

$draftareaid = file_get_submitted_draft_itemid('attachments');
file_prepare_draft_area($draftareaid, $PAGE->context->id,
...
$data['content']['itemid'] = $textareaid;
$data['attachments'] = $draftareaid;
$data['attachmentsintext'] = $textareaid;
$mform->set_data($data);
...
if ($data = $mform->get_data()) {
    ...
    if (!empty($data->discard)) {
        ...
        $fs->delete_area_files($PAGE->context->id, 'local_mail', 'attachmentintext', $message->id());
        $message->discard();
    ...
    $content = file_save_draft_area_files($data->attachmentsintext, $PAGE->context->id,
                                            'local_mail', 'attachmentintext', $message->id(),
                                            mail_compose_form::file_options(),
                                            $data->content['text']);

    file_save_draft_area_files($data->attachments, $PAGE->context->id,
                                          'local_mail', 'message', $message->id(),
                                          mail_compose_form::file_options());

    $attachments = false;
    if ($fs->get_area_files($PAGE->context->id, 'local_mail', 'message', $message->id(), 'filename', false) ||
            $fs->get_area_files($PAGE->context->id, 'local_mail', 'attachmentintext', $message->id(), 'filename', false)) {
        $attachments = true;
    }
