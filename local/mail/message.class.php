    ...
    static public function count_menu($userid) {
        ...
        foreach ($records as $record) {
            if ($record->type == 'inbox' and $record->unread) {
			    ...
			    // This part of the code is commented to allow user to view messages in a course where they are not enrolled.
				// Otherwise, they could end up with unread messages in their inbox but with no way to read them.
			    // HACK BEGINNING
...
//                }
    ...
    static public function fetch_many(array $ids) {
        ...
        // HACK FOR COURSUCP 2016-2017
        // This part of the code is commented to allow the user to read mail in course where they are not enrolled.
		// HACK END
...
//        }
    ...
    private function match($userid, $pattern) {
        ...
        $content = file_rewrite_pluginfile_urls($this->content, 'pluginfile.php', $context->id,
                                            'local_mail', 'attachmentintext', $this->id);
