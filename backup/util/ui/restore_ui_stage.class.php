        ...
        if ($this->stageform === null) {
            ...
            foreach ($tasks as &$task) {
                ...
                if ($task instanceof restore_root_task) {
                    ...
                    foreach ($settings as &$setting) {
                        //BRICE : pour simplifier la procédure de restauration
                        if ($PAGE->get_transfer()) {
                            $settingname = $setting->get_name();
                            //echo "$settingname<br>";
                            if (($settingname == "activities")||($settingname == "questionbank")) {
                                $setting->set_value(true);                            
                                $setting->set_visibility(0);
                            } else if (
                                    ($settingname == "imscc11") || ($settingname == "users") || ($settingname == "blocks")
                                    || ($settingname == "filters") || ($settingname == "calendarevents") || ($settingname == "groups")
                                    ) {
...
class restore_ui_stage_complete extends restore_ui_stage_process {
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        //BRICE : si c'est un transfert de cours, il est maintenant terminé
        $transfercourse = $DB->set_field('oldcourses', 'beingtransferedby', 0, array('beingtransferedby' => $USER->id));
        //FIN
