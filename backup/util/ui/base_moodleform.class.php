...
abstract class base_moodleform extends moodleform {
    ...
    public function add_settings(array $settingstasks) {
        ...
        foreach ($settingstasks as $st) {
            ...
            //BRICE
            if ($PAGE->get_transfer()) {
                $settingname = $setting->get_name(); 
                $settingstatus = $setting->get_status();
                //echo "$settingname $settingstatus<br>";
                if ($settingname == "activities") {
                    $setting->set_value(true);
                    /*$setting->set_visibility(0);
                    $this->_form->addElement('html', 
                            "Ces réglages sont masqués pour simplifier la procédure. Cliquez <span style='color:red;font-weight:bold'>2 fois</span> sur le bouton <strong>Suivant</strong> ci-dessous puis sur <strong>Effectuer la restauration</strong>."); */
                } else if (($settingname == "course_fullname")||
                            ($settingname == "course_shortname")||
                            ($settingname == "course_startdate")||
                            ($settingname == "keep_roles_and_enrolments")||
                            ($settingname == "keep_groups_and_groupings")||
                            ($settingname == "overwrite_conf")) {
                    $setting->set_visibility(0);                
                } else if (($settingname == "users")||
                            ($settingname == "enrol_migratetomanual")||
                            ($settingname == "role_assignments")||
                            ($settingname == "blocks")||
                            ($settingname == "filters")||
                            ($settingname == "badges")||
                            ($settingname == "calendarevents")||
                            ($settingname == "userscompletion")||
                            ($settingname == "logs")||
                            ($settingname == "grade_histories")||
                            ($settingname == "groups")||
