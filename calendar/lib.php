...
class calendar_event {
    ...
    public function update($data, $checkcapability=true) {
        ...
        if (empty($this->properties->id) || $this->properties->id < 1) {
			...
			if($this->properties->courseid ==1)
			{
				$count = "select count(id) as nbr from mdl_abonnement";
				$rescount =$DB->get_record_sql($count);
				if($rescount->nbr >=1)
				{
					$sqlsubscribe = "select id,userid from mdl_abonnement";
					$ressubscribe = $DB->get_records_sql($sqlsubscribe);
					$headers = 'From: noreply@cours.u-cergy.fr'."\r\n".'MIME-Version: 1.0'."\r\n".
							   'Reply-To: noreply@cours.u-cergy.fr'."\r\n".
							   'Content-type: text/html; charset=utf-8'.'\r\n'.
							   'X-Mailer: PHP/' . phpversion();
					foreach($ressubscribe as $subscribe)
					{
						$sqldata ="select email,firstname,lastname from mdl_user where id =$subscribe->userid";
						$resdata =$DB->get_record_sql($sqldata);
						$data = $this->properties->id;
						$sqlevent = "SELECT name FROM mdl_event WHERE courseid =1 and id =$data";
						$resevent = $DB->get_record_sql($sqlevent);
						$message = "Bonjour $resdata->firstname $resdata->lastname,\nUne nouvelle actualité dans CoursUCP $resevent->name.\nCoursUCP, votre plateforme pédagogique";
						...
						mail($resdata->email, 'Nouvelles CoursUCP', $message, $headers);
					}
				}
			}
