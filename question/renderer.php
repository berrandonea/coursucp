...
class core_question_bank_renderer extends plugin_renderer_base {
    ...
    public function qbank_chooser($real, $fake, $course, $hiddenparams) {
        ...
        $classictypenames = array("qtype_match", "qtype_multichoice", "qtype_gapfill", "qtype_numerical", "qtype_shortanswer", "qtype_truefalse");
        
        /*
        qtype_match                 Appariement
        qtype_calculated            Calculée
        qtype_calculatedmulti       Calculée à choix multiples
        qtype_calculatedsimple      Calculée simple
        qtype_multichoice           Choix multiple
        qtype_essay                 Composition
        qtype_gapfill               Texte à trous
        qtype_ddimageortext         Glisser-déposer sur une image
        qtype_ddmarker              Marqueurs à glisser-déposer
        qtype_numerical             Numérique
        qtype_pmatch                Pattern match
        qtype_pmatchjme             Pattern match with molecular editor
        qtype_multianswer           Cloze : texte à trous sofistiqué
        qtype_randomsamatch         Question de correspondance à réponse courte
        qtype_shortanswer           Réponse courte
        qtype_regexp                Réponse courte de type expression régulière
        qtype_gapselect             Texte à trous avec menu déroulants
        qtype_truefalse             Vrai/Faux
         * 
         * classique : qtype_match, qtype_multichoice, qtype_gapfill, qtype_numerical, qtype_shortanswer, qtype_truefalse
         * 
         * 
        */
        ...
        foreach($real as $realtype) {            
            $realtypeclass = get_class($realtype);           
            if (in_array($realtypeclass, $classictypenames)) {
                $classictypes[$nbclassictypes] = $realtype;
                $nbclassictypes++;
            } else {
                $othertypes[$nbothertypes] = $realtype;
                $nbothertypes++;
            }            
        }
        ...
        */
        ...
        //BRICE
        $formcontent .= html_writer::div("QUESTIONS CLASSIQUES", 'option moduletypetitle');        
        $formcontent .= $this->qbank_chooser_types($classictypes);
        $formcontent .= html_writer::div('', 'separator');
        
        // Then fake questions.
        $formcontent .= html_writer::div("CONSIGNES", 'option moduletypetitle');
        $formcontent .= $this->qbank_chooser_types($fake);
                
        //BRICE
        $formcontent .= html_writer::div('', 'separator');
        $formcontent .= html_writer::div("PLUS DE QUESTIONS", 'option moduletypetitle');        
        $formcontent .= $this->qbank_chooser_types($othertypes);
