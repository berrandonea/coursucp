 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbehaviour_ucpcbm_type_test extends qbehaviour_walkthrough_test_base {

    /** @var qbehaviour_ucpcbm_type */
    protected $behaviourtype;

    public function setUp() {
        parent::setUp();
        $this->behaviourtype = question_engine::get_behaviour_type('ucpcbm');
    }
    ...
    public function test_summarise_usage_max_mark_1() {
        ...
        $this->quba->set_preferred_behaviour('ucpcbm');
        ...
        // Verify.
        $this->assertContains(get_string('breakdownbycertainty', 'qbehaviour_ucpcbm'),
                $summarydata['qbehaviour_cbm_judgement_heading']['content']);

        $this->assertContains('100%',
                $summarydata['qbehaviour_cbm_judgement3']['content']);
        $this->assertContains(get_string('judgementok', 'qbehaviour_ucpcbm'),
                $summarydata['qbehaviour_cbm_judgement3']['content']);

        $this->assertContains('50%',
                $summarydata['qbehaviour_cbm_judgement2']['content']);
        $this->assertContains(get_string('slightlyoverconfident', 'qbehaviour_ucpcbm'),
                $summarydata['qbehaviour_cbm_judgement2']['content']);

        $this->assertContains(get_string('noquestions', 'qbehaviour_ucpcbm'),
