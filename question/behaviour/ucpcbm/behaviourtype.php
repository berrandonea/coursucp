 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qbehaviour_ucpcbm_type extends qbehaviour_deferredfeedback_type {
    public function adjust_random_guess_score($fraction) {
        return question_ucpcbm::adjust_fraction($fraction, question_ucpcbm::default_certainty());
    }
    ...
    public function summarise_usage(question_usage_by_activity $quba, question_display_options $options) {
        ...
        $notansweredweight = 0;
        $attemptcount = array(
            question_ucpcbm::HIGH => 0,
            question_ucpcbm::MED  => 0,
            question_ucpcbm::LOW  => 0,
        );
        $totalweight = array(
            question_ucpcbm::HIGH => 0,
            question_ucpcbm::MED  => 0,
        ...
        $summarydata['qbehaviour_cbm_grade_explanation'] = array(
            'title' => '',
            'content' => html_writer::tag('i', get_string('cbmgradeexplanation', 'qbehaviour_ucpcbm')) .
                    $OUTPUT->help_icon('cbmgrades', 'qbehaviour_ucpcbm'),
        );
        ...
        if ($notansweredcount && array_sum($attemptcount) > 0) {
            ...
            $summarydata['qbehaviour_cbm_answered_quiz_heading'] = array(
                'title' => '',
                'content' => html_writer::tag('h3',
                        get_string('foransweredquestions', 'qbehaviour_ucpcbm', $totalquestions),
                        array('class' => 'qbehaviour_ucpcbm_summary_heading')),
            );
        ...
        $summarydata['qbehaviour_cbm_judgement_heading'] = array(
            'title' => '',
            'content' => html_writer::tag('h3', get_string('breakdownbycertainty', 'qbehaviour_ucpcbm'),
                    array('class' => 'qbehaviour_ucpcbm_summary_heading')),
        );

        foreach ($attemptcount as $certainty => $count) {
            $key   = 'qbehaviour_cbm_judgement' . $certainty;
            $title = question_ucpcbm::get_short_string($certainty);

            if ($count == 0) {
                $summarydata[$key] = array(
                    'title' => $title,
                    'content' => get_string('noquestions', 'qbehaviour_ucpcbm'),
                );
                continue;
            }

            $lowerlimit = question_ucpcbm::optimal_probablility_low($certainty);
            $upperlimit = question_ucpcbm::optimal_probablility_high($certainty);
            ...
            $a->fraction       = html_writer::tag('span', $this->format_probability($fraction),
                    array('class' => 'qbehaviour_ucpcbm_actual_percentage'));
            ...
            $a->judgement = html_writer::tag('span', get_string($judgement, 'qbehaviour_ucpcbm'),
                    array('class' => 'qbehaviour_ucpcbm_' . $judgement));

            $summarydata[$key] = array(
                'title' => $title,
                'content' => get_string('judgementsummary', 'qbehaviour_ucpcbm', $a),
            );
        }
    ...
    public function calculate_bonus($total, $accuracy) {
        $expectedforaccuracy = max(
            $accuracy * question_ucpcbm::adjust_fraction(1, question_ucpcbm::LOW) +
                (1 - $accuracy) * question_ucpcbm::adjust_fraction(0, question_ucpcbm::LOW),
            $accuracy * question_ucpcbm::adjust_fraction(1, question_ucpcbm::MED) +
                (1 - $accuracy) * question_ucpcbm::adjust_fraction(0, question_ucpcbm::MED),
            $accuracy * question_ucpcbm::adjust_fraction(1, question_ucpcbm::HIGH) +
                (1 - $accuracy) * question_ucpcbm::adjust_fraction(0, question_ucpcbm::HIGH)
        );
