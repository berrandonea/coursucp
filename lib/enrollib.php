...
function enrol_add_course_navigation(navigation_node $coursenode, $course) {
    ...
    if (($course->groupmode || !$course->groupmodeforce) && has_capability('moodle/course:managegroups', $coursecontext)) {
     ...
     /* BRICE pour cacher "Permissions" dans le menu "Administration du cours" 
     ...
     /* BRICE pour cacher "Autres utilisateurs" dans le menu "Administration du cours" 
    ...
    if ($course->id != SITEID) {
        if (isguestuser() or !isloggedin()) {
            ...
            foreach ($instances as $instance) {
                ...
                //* BRICE pour cacher "Me désinscrire de" dans le menu "Administration du cours" 
...
abstract class enrol_plugin {
    ...
    public function unenrol_user(stdClass $instance, $userid) {
        ...
        //BRICE : on enregistre la désinscription dans mdl_unenroled
        $unenrolment = new StdClass();
        $unenrolment->userid = $userid;
        $unenrolment->courseid = $courseid;
        $unenrolment->unenrolerid = $USER->id;
        $unenrolment->unenroled_at = time();
        $unenrolment->id = $DB->insert_record('unenroled', $unenrolment);         
        //FIN
