...
class filter_manager {
	...
	//BRICE
	global $PAGE;
	$filterchain = $this->get_text_filters($context);
	if (strpos($PAGE->url, '/mod/realtimequiz/edit.php')) {
	    $num = 0;
	    foreach ($filterchain as $filter) {
		if (get_class($filter) == 'filter_tex') {
		    unset($filterchain[$num]);
		}
		$num++;
	    }
	}
        $text = $this->apply_filter_chain($text, $filterchain, $options);
	//FIN
//BRICE        $text = $this->apply_filter_chain($text, $this->get_text_filters($context), $options);
