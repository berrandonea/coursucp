...
//BRICE : dans ce fichier, j'ai remplacé tous les error_log() par des ucp_log()
function ucp_log($logstring) {
    global $USER, $COURSE, $DB;    
    $now = time();
    error_log($logstring);
    
    //$logstrings = explode(" Debug: ", $logstring);
    
    $logstring = addslashes($logstring);   
//	echo "$logstring<br>; exit;    
    
/*    $sql = "INSERT INTO mdl_error (error_at, userid, courseid, message) VALUES ($now, $USER->id, $COURSE->id, '$logstring')";
    $DB->execute($sql);*/
    
    
    $loggederror = new stdClass();
    $loggederror->error_at = $now;
    $loggederror->userid = $USER->id;
    $loggederror->courseid = $COURSE->id;
    $loggederror->message = $logstring;            
    //$loggederror->id = $DB->insert_record('error', $loggederror);        
    
    
    
    
    
    //$logstring .= "***$USER->firstname $USER->lastname ($USER->id, $USER->email) dans le cours $COURSE->id***\n\n";
...
function abort_all_db_transactions() {
    ...
    if ($DB && $DB->is_transaction_started()) {
        ucp_log('Database transaction aborted automatically in ' . $CFG->dirroot . $SCRIPT);
