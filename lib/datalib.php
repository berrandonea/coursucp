...
function get_courses_search($searchterms, $sort, $page, $recordsperpage, &$totalcount) {
   ...
   // Thanks Oracle for your non-ansi concat and type limits in coalesce. MDL-29912
    if ($DB->get_dbfamily() == 'oracle') {
        $concat = "(c.summary|| ' ' || c.fullname || ' ' || c.idnumber || ' ' || c.shortname"
                . " || ' ' || u.firstname || ' ' || u.lastname" //BRICE : chercher aussi dans les noms des enseignants et enseignants non éditeurs
                . ")";
    } else {
    ...
    */
    ...
    //BRICE : chercher aussi dans le nom des enseignants
    $params['contextlevel'] = CONTEXT_COURSE;
    $sql = "SELECT c.* $ccselect "
            . "FROM {course} c, {user} u, {role_assignments} ra, {context} ctx "
            . "WHERE $searchcond AND c.id <> ".SITEID." "
            . "AND ctx.instanceid = c.id AND ctx.contextlevel = :contextlevel "
            . "AND ra.contextid = ctx.id AND (ra.roleid = 3 OR ra.roleid = 4) AND ra.userid = u.id "
            . "ORDER BY $sort";
    $rs = $DB->get_recordset_sql($sql, $params);
    //FIN
