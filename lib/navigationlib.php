...
class settings_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        if (isloggedin() && !isguestuser() && (!isset($SESSION->load_navigation_admin) || $SESSION->load_navigation_admin)) {
            ...
            if ($SESSION->load_navigation_admin && !$isadminpage) {
                ...
                if (has_capability('moodle/user:loginas', context_system::instance())) { //BRICE : Afficher "Administration du site seulement pour les admins et les ingénieurs pédagogiques
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if (has_capability('moodle/site:viewreports', $coursecontext)) { // Basic capability for listing of reports.
            ...
            foreach ($reports as $reportfunction) {                
                //print_object($reportfunction);
                if (($reportfunction != 'report_stats_extend_navigation_course')&&($reportfunction != 'report_engagement_extend_navigation_course')) { //BRICE
                    $reportfunction($reportnav, $course, $coursecontext);
                } //BRICE                
            }
        }
        ...
        if (has_capability('moodle/role:switchroles', $coursecontext)) {
            ...
            if (is_array($availableroles)) {
                foreach ($availableroles as $key=>$role) {
                    ...
                    if ($key == 4 || $key == 5) //BRICE : on affiche uniquement les rôles Enseignant non éditeur et étudiant
                    $roles[$key] = $role;
                }
            }
        }
    ...
    protected function load_category_settings() {
        ...
        if (can_edit_in_category($catcontext->instanceid)) {
			// Laurent
			// Commented to hide the manage category button in the category block
			// HACK BEGINNING
        ...
        if (has_capability('moodle/course:create', $catcontext)) {
			// Laurent
			// Commented to hide the restore course button in the category block
			// HACK BEGINNING
