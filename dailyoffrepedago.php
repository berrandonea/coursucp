...
foreach($queryvet as $vet){
    ...
    if ($vetresult) {
        $sql = "SELECT name FROM mdl_course_categories WHERE idnumber = '".$codevet."'";
        ...
        if ($currentvetname != $updatedvetname) {
            ...
            $sql = "UPDATE mdl_course_categories SET name = \"".$updatedvetname."\" WHERE idnumber = '".$codevet."'";
            echo "$sql\n";
            $DB->execute($sql);
        }
    }
    ...
    $sql = "SELECT id FROM mdl_cat_demandecours WHERE code = '$codevet'";
    $vetresult = $DB->get_record_sql($sql);
    if ($vetresult) {
        $vetid = $vetresult->id;
        ...
        $sql = "SELECT id FROM mdl_cat_demandecours WHERE code = '$ufr'";
        $ufrresult = $DB->get_record_sql($sql);

        if ($ufrresult) {
            $ufrid = $ufrresult->id;
        } else {
            $sql = "SELECT name FROM mdl_ufr WHERE code = '$ufr'";
            $ufrshortname = $DB->get_record_sql($sql)->name;
            echo "Nouvelle composante: $ufr\n";
            $newufr = new stdClass();
    ...
    foreach($queryelp as $elp){
        ...
        //echo "codelp: $codeelp\n";
        $sql = "SELECT id FROM mdl_cat_demandecours WHERE code = '$codevet-$codeelp'";
        //echo "$sql\n";
        ...
        if ($elpresult) {
            ...
            //echo $sql."<br/>";
            echo "Nouvel ELP: $codevet-$codeelp<br/>\n";
            /*$newelp = new stdClass();
            $newelp->name = $updatedelpname;
            $newelp->code = $codevet."-".$codeelp;
            $newelp->parent_id = $vetid;
            $DB->insert_record('cat_demandecours', $newelp);*/
            $sql = "INSERT INTO mdl_cat_demandecours (name, code, parent_id) VALUES (\"".addslashes($updatedelpname)."\", \"$codevet-$codeelp\", $vetid)";
            echo "$sql\n";
            $DB->execute($sql);
        }
    }
