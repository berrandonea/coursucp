...
//BRICE require_once($CFG->libdir . '/conditionlib.php');
...
$PAGE->set_url('/course/editsp.php', array('id'=>$id, 'sr'=> $sectionreturn));

//$section = $DB->get_record('course_sections', array('id' => $id), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id' => $id), '*', MUST_EXIST);

if(!empty($sectionid)){    
    $section = $DB->get_record('sp_section', array('section' => $sectionid));
    $sectionnum = $section->section;
    $sqlsp = 'SELECT course,name,description,result as Resultatattendu,du as datedu, au as dateau, nbgroup as maxgroupe FROM mdl_situation_problemes where id='.$section->sp;
    $sp = $DB->execute($sqlsp,null);    
} else {    
    $sectionnum = 0;
}


//$sp = $DB->get_record('situation_problemes', array('id' => $section->sp));
...
$mform = $courseformat->editsp_form('ajoutSp.php?id='.$course->id, array('sp'=>$spid,'cs' => $sectioninfo, 'editoroptions' => $editoroptions));
...
$stredit      = 'Ajouter une situation problème';
$strsummaryof = 'Ajouter une situation problème';
