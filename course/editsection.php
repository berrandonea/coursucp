...
$sqlsection = "select sequence from mdl_course_sections where course = $course->id and section = $sectionnum";
$resultsection = $DB->get_recordset_sql($sqlsection);
$existcontenusection = $DB->record_exists_sql($sqlsection);
foreach ($resultsection as $sec)
{
	if($sec->sequence && $deletesection)
	{
...
if($sec->sequence && $deletesection)
	{
		echo $OUTPUT->heading("Suppression d'une section non vide");
		echo "<strong>Merci de vider tout le contenu de la section afin de pouvoir la supprimer </strong><br>";
		echo "<a href='view.php?id=$course->id'>Revenir au cours</a>";
		echo $OUTPUT->footer();
		exit;
	}
