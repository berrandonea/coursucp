...
class course_edit_form extends moodleform {
    ...
    function definition() {
        ...
        // Commenté par BRICE $mform->addElement('text','fullname', get_string('fullnamecourse'),'maxlength="254" size="50"');

        $mform->addHelpButton('fullname', 'fullnamecourse');
        $mform->addRule('fullname', get_string('missingfullname'), 'required', null, 'client');
        $mform->setType('fullname', PARAM_TEXT);

        /* BRICE - ON FREEZE LE CHAMP SI L'UTILISATEUR N'EST PAS ADMIN ET ON LE REMPLIT AVEC LES ELEMENTS ENVOYES EN GET SI C'EST UNE CREATION*/
        $paramtitrecours = optional_param('titrecours', '', PARAM_TEXT);
        if($paramtitrecours) {
        	$mform->setDefault('fullname', $paramtitrecours);
        }//fin
        ...
        //BRICE
        $paramcodecours = optional_param('codecours', '', PARAM_TEXT);
        if($paramcodecours) {
        	$mform->setConstant('shortname', $paramcodecours);
        }//fin
        
        if (!empty($course->id) and !has_capability('moodle/course:changeshortname', $coursecontext)
        		and $category->id != $CFG->catbrouillonsid //BRICE
        		) {
        ...
        if (empty($course->id)) {
        	...
        	//Pour les enseignants, l'id de la catégorie est imposé par un hidden
        	if (!is_siteadmin() && !$manager){
        		$mform->addElement('hidden', 'category', get_string('coursecategory'),array("readonly" => "readonly"));
        	} else {
        		//Les admins et les ingénieurs pédagogiques peuvent choisir dans quelle catégorie ils créent le cours
        		$mform->addElement('select', 'category', get_string('coursecategory'), $displaylist);
        	}
        	 
        	// $mform->addHelpButton('category', 'coursecategory');
        	 
        	//$paramcat = optional_param("cat", 0, PARAM_INT);
        	//if (!$paramcat) {
        		$idcat = $category->id;//optional_param("category", 0, PARAM_INT);
        	/*} else {
        		/*ON RECUPERE AUTOMATIQUEMENT L'ID DE LA CATEGORIE ENVOYEE EN GET CAT
        		$categoryy = $DB->get_record('course_categories', array("idnumber" => $paramcat));
        		$idcat = $categoryy->id;
        	}*/
                
        	$mform->setDefault('category', $idcat);        	
        	
        } else { //Modification d'un cours existant
            /* Commenté par BRICE
        ...
        /* ON FREEZE LE CHAMP SI L'UTILISATEUR EST PAS ADMIN ET ON LE REMPLIT AVEC LES ELEMENTS ENVOYES EN GET SI C'EST UNE CREATION*/
        if($paramcodecours) {
        	$mform->setConstant('idnumber', $paramcodecours);
        }
        //FIN
