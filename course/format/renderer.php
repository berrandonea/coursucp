...
abstract class format_section_renderer_base extends plugin_renderer_base {
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {            
            ...
            if (!$PAGE->user_is_editing() && $course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) {
                ...
                if ($thissection->uservisible) {                    
                    //BRICE Si on est en mode édition, on regarde si la section est une situation problème
                    if ($PAGE->user_is_editing()) {
                        global $DB, $CFG;
                        
                        $sql = "SELECT spid FROM mdl_course_sections WHERE id = $thissection->id";
                        $spid = $DB->get_record_sql($sql)->spid;
                        
                        if ($spid) {
                            echo "<a href='$CFG->wwwroot/group/spmembers.php?id=$course->id&spid=$spid'>Modifier les groupes</a><br><br>";
                        }
                    }                    
                    //FIN
        ...
        if ($PAGE->user_is_editing() and has_capability('moodle/course:update', $context)) {
            ...
            echo '<a href='.$url.'><input type="submit" value="Ajouter une section"></a>';
            
/*            //BRICE            
            $url = new moodle_url('/course/editsp.php',
            array('id' => $course->id, 'sp' => 1));
            echo '&nbsp;&nbsp;&nbsp;<a href='.$url.'><button>Ajouter une situation problème</button></a>';
            //FIN*/
