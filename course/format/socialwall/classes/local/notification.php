...
class notification {
    ...
    protected function create_message($notification, $course) {
        ...
        $urlnotification = new \moodle_url('/course/format/socialwall/pages/editnotification.php',
                array('courseid' => $notification->courseid));

        $notificationlink = \html_writer::link($urlnotification,
                get_string('deactivatenotificationlink', 'format_socialwall'));

        $notificationtext = get_string('deactivatenotification', 'format_socialwall', $notificationlink);
        ...
        $eventdata->fullmessage = $messagecontent . ', ' . $url->out() . $urlnotification->out();

        $eventdata->fullmessageformat = FORMAT_PLAIN;   // Text format.
        $courselink = \html_writer::link($url, get_string('nf_gotocourse', 'format_socialwall'));
        $eventdata->fullmessagehtml = $messagecontent . ', ' . $courselink . ' ' . $notificationtext;
...
// This is to set a default value for notification for all users in course.
// This does not touch the user preferences if he have already set them.
// However, another hack in code prevent him to change them.
// This function is called by instant cron so changes should happen the next time cron runs.
// Hack for CoursUCP 2016-2017
// THIS IS BAD PRACTICE, hacks shouldn't be done in the source code.
// HACK BEGINNING
        ...
        $roleassignments = $DB->get_records('role_assignments', array('contextid' => $context->id));

        foreach ($roleassignments as $roleassignment) {

            if (!$DB->record_exists('format_socialwall_nfsettings',
                    array('courseid' => $courseid, 'userid' => $roleassignment->userid))) {

                $recordtoinsert['courseid'] = $courseid;
                $recordtoinsert['userid'] = $roleassignment->userid;
                $recordtoinsert['notificationtype'] = 1;

                $DB->insert_record('format_socialwall_nfsettings', $recordtoinsert);
            }
        }
