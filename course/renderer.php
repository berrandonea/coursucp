...
class core_course_renderer extends plugin_renderer_base {
    ...
    */
        ...
        $commonactivities = array_filter($modules, create_function('$mod', 
                'return ($mod->name == "folder" || $mod->name == "assign" || 
			$mod->name == "quiz" || 
			$mod->name == "page" || $mod->name == "etherpadlite" ||
			$mod->name == "label");'));
        if (count($commonactivities)) {
            $formcontent .= $this->course_modchooser_title('commonactivities');
            $formcontent .= $this->course_modchooser_module_types($commonactivities);
        }

        // Plus d'activités : 
        $moreactivities = array_filter($modules, create_function('$mod', 
                'return ($mod->name == "forum" || $mod->name == "resource" || $mod->name == "glossary" || $mod->name == "workshop" ||
			$mod->name == "chat" || $mod->name == "lesson" || $mod->name == "adaptivequiz" || $mod->name == "game" || $mod->name == "hvp" || $mod->name == "offlinequiz");'));
        if (count($moreactivities)) {
            $formcontent .= $this->course_modchooser_title('moreactivities');
            $formcontent .= $this->course_modchooser_module_types($moreactivities);
        }
                
        // Encore plus d'activités : 
        $moremoreactivities = array_filter($modules, create_function('$mod', 
                'return ($mod->name == "url" || $mod->name == "wiziq" || $mod->name == "ipal" || $mod->name == "realtimequiz" || 
			$mod->name == "taskchain" || $mod->name == "choicegroup" || $mod->name == "feedback" || $mod->name == "wiki" ||
			$mod->name == "bigbluebuttonbn" || $mod->name == "scorm" || $mod->name == "survey" || $mod->name == "autoattendmod" ||
			$mod->name == "book" || $mod->name == "mindmap" || $mod->name == "scheduler" || $mod->name == "depotetudiant" || 
                        $mod->name == "lti" || $mod->name == "qcreate" || $mod->name == "data" ||
			$mod->name == "recordingsbn" || $mod->name == "vpl");'));
        if (count($moremoreactivities)) {
            $formcontent .= $this->course_modchooser_title('moremoreactivities');
            $formcontent .= $this->course_modchooser_module_types($moremoreactivities);
        }
    ...
    protected function course_modchooser_module($module, $classes = array('option')) {
        ...
        if (!isset($module->types)) {
            $output .= html_writer::start_tag('td', array('width' => '5px')); //BRICE
            $output .= html_writer::tag('input', '', array('type' => 'radio',
                    'name' => 'jumplink', 'id' => 'module_' . $module->name, 'value' => $module->link));
            $output .= html_writer::end_tag('td'); //BRICE
        }

        $output .= html_writer::start_tag('td', array('width' => '20px')); //BRICE
        ...
        $output .= html_writer::start_tag('td', array('style' => 'text-align:left')); //BRICE
    ...
    protected function course_modchooser_title($title, $identifier = null) {
        ...
        if($title == "commonactivities") {
            $module->title = "Activités classiques";
        } else if($title == "moreactivities") {
            $module->title = "Plus d'activités";
        } else if($title == "moremoreactivities") {
            $module->title = "Encore plus d'activités";
        } else {
            $module->title = get_string($title, $identifier);
        }
        $module->help = '';
    ...
    function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        $sectionname = "<span style ='color : red'>$sectionname</span>";
    ...
    function course_search_form($value = '', $format = 'plain') {
        ...
        $output .= html_writer::tag('label', $strsearchcourses.': &nbsp;', array('for' => $inputid)); //BRICE
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => $inputid,
            'size' => 30/*BRICE*/, 'name' => 'search', 'value' => s($value)));
	$output .= " "; //BRICE
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        //BRICE
        $idnumberstart = substr($course->idnumber, 0, 6);
        switch ($idnumberstart) {
			case 'Y2017-':
			    $catlabel = '2017-2018';
			    break;
			case 'COLLAB':
        ...
        $content .= "<strong>$catlabel</strong>";
        //FIN                                            
    ...
    public function course_category($category) {
        ...
        if (can_edit_in_category($category)) {
			...
			// Laurent
			// This is commented to hide the manage course button.
			// HACK BEGINNING
		...
		// Laurent
		// This is commented to hide the create course button.
		// HACK BEGINNING
