...
function createvetifnew($codevet, $nomvet, $ufr = null, $level = null) {
	...
	//echo "createvetifnew($codevet, $nomvet)<br>";
	$sql = "SELECT id, COUNT(id) as vetexists FROM mdl_course_categories WHERE idnumber = '$yearlycodevet'";
	//echo "sql: $sql<br>";
	$vetresult = $DB->get_record_sql($sql);
	//echo "$vetresult->vetexists<br>";
	if ($vetresult->vetexists) {
		//echo "Cette catégorie-vet existe déjà<br>";
		$vetid = $vetresult->id;
	} else {
		...
		echo "level: $level<br>";
		echo "testlicence: $testlicence<br>";
        
		if (($level == "L1")||($level == "L2")||($level == "L3")||($level == "LP")||($level == "M1")||($level == "M2")) {
			$levelcode = $ufr.$level;
			$levelname = $level;
		} else if ($level == "DU") {
			$levelcode = $ufr."DU";
		...
		//Si le niveau en question n'existe pas encore dans Moodle, on le crée
		$sql = "SELECT id FROM mdl_course_categories WHERE idnumber = 'Y2017-$levelcode'";
                echo "$sql<br>";
		...
		if ($levelresult) {
			...
			//id de l'UFR correspondante
			$sql = "SELECT id FROM mdl_course_categories WHERE idnumber = 'Y2017-$ufr'";
			$ufrid = $DB->get_record_sql($sql)->id;
			$leveldata = array('name'=>$levelname,'idnumber'=>"Y2017-$levelcode", 'parent'=>$ufrid, 'visible'=>1);
			$levelobject = coursecat::create($leveldata);
			$levelid = $levelobject->id;
		}
		echo "levelid: $levelid<br>";
		$completename = "($codevet) $nomvet";
		$vetdata = array('name'=>$completename,'idnumber'=>"Y2017-$codevet", 'parent'=>$levelid, 'visible'=>1);
                //print_object($vetdata);
		$vetobject = coursecat::create($vetdata);
		$vetid = $vetobject->id;
	}
	//echo "vetid: $vetid<br>";
...
//BRICE : situation problème
function editsp_form($action, $customdata = array()) {
    global $CFG;
    require_once($CFG->dirroot. '/course/editsp_form.php');
    $context = context_course::instance($this->courseid);
    if (!array_key_exists('course', $customdata)) {
        $customdata['course'] = $this->get_course();
    }
    return new editsp_form($action, $customdata);
}
...
function course_create_sections_if_missing($courseorid, $sections) {
    ...
    //BRICE
    //On consulte le format du cours
    $sql = "SELECT format FROM mdl_course WHERE id = $courseorid";
    $courseformat = $DB->get_record_sql($sql)->format;
    //FIN
    ...
    foreach ($sections as $sectionnum) {
        if (!in_array($sectionnum, $existing)) {
            ...
            //BRICE
            //On récupère l'ancienne séquence de cette section (s'il y en a une)
            $sql = "SELECT sequence FROM mdl_course_sections WHERE id = $sectionid";
            $oldsequence = $DB->get_record_sql($sql)->sequence;
                        
            //Si c'est la section 0 
            $sql = "SELECT section FROM mdl_course_sections WHERE id = $sectionid";
            $sectionnumwithincourse = $DB->get_record_sql($sql)->section;            
                        
            if ($sectionnumwithincourse == 0) {
                //Titre et texte explicatif
	        $DB->update_record('course_sections',array('id'=>$sectionid,'name'=>'Description générale du cours', 
		'summary'=>'Pour éditer le cours, cliquez sur le bouton noir "Activer le mode édition" en haut à droite. Vous verrez alors apparaître sous ce texte un petit engrenage gris. '
                    . 'En cliquant dessus, vous pourrez remplacer ce texte par autre chose, par exemple par une liste des pré-requis et/ou des objectifs de ce cours, ou encore par une courte introduction.<br>'));
                
                //Création du forum des messages importants
                $newsforumid = $DB->insert_record('forum',array('course'=>$courseorid,'name'=>'Messages importants','type'=>'news','intro'=>'Nouvelles diverses et annonces',
                    'maxattachements'=>1,'forcesubscribe'=>1,'trackingtype'=>1,'timemodified'=>time()));
                $modulenewsforumid = $DB->insert_record('course_modules',array('course'=>$courseorid,'module'=>20,'instance'=>$newsforumid,'section'=>0,'added'=> time(), 'indent'=>0)); 
                
                //Rendez-vous
                $schedulintro = '<p>Ici, vous pouvez convenir d\'un rendez-vous avec les enseignants.</p>';
                $schedulid = $DB->insert_record('scheduler',array('course'=>$courseorid,'name'=>'Prendre un rendez-vous', 'intro'=> $schedulintro,'timemodified'=>  time()));
                $moduleschedulid = $DB->insert_record('course_modules',array('course'=>$courseorid,'module'=>36,'instance'=>$schedulid,'section'=>$sectionid,'added'=> time(), 'indent'=>0)); 


                $sequence = "$modulenewsforumid,$moduleschedulid";

                if ($courseformat == 'onesection') {
                    $DB->update_record('course_sections',array('id'=>$sectionid,'name'=>'Contenu du cours'));
                    // Création d'un quiz                       
                    $quizid = $DB->insert_record('quiz',array('course'=>$courseorid,'name'=> 'Test', 'intro'=> '','timecreated'=> time(), 'timemodified'=>time(), 'questions'=>0));
                    $modulequizid = $DB->insert_record('course_modules',array('course'=>$courseorid,'module'=>16,'instance'=>$quizid,'section'=>$sectionid,'added'=> time(), 'indent'=>0));
                    $sequence .= ",$modulequizid";                    
                }
                $DB->update_record('course_sections',array('id'=>$sectionid,'sequence'=>$sequence));
...
function create_course($data, $editoroptions = NULL) {
    ...
    // Setup the blocks
    //BRICE blocks_add_default_course_blocks($course);
