...
if ($hasfooter) { ?>
    <footer id="page-footer">
        ...
        <!-- BRICE
        <p class="helplink"><?php //BRICE echo page_doc_link(get_string('moodledocslink')) ?></p>
        <?php
        //Commenté par BRICE echo $OUTPUT->login_info();
        ?>
        <p>
            <a href="http://moodle.org" title="Moodle">
                <img src="<?php //BRICE echo $OUTPUT->pix_url('moodle-logo','theme')?>" alt="Moodle logo" />
            </a>
        </p>
        -->
...
<?php
    //BRICE
    $accordion = $PAGE->theme->settings->accordionBlocks;
    if ($PAGE->user_is_editing() || ($PAGE->pagelayout == 'frontpage')) {
        $accordion = false;
    }

    if (($PAGE->pagetype == 'mod-assign-mod')||($PAGE->pagetype == 'mod-workshop-mod')) {
        $accordion = false;
    }

    if (($PAGE->pagetype == 'mod-assign-view')||($PAGE->pagetype == 'mod-workshop-view')) {
        if (strpos($PAGE->url, 'action=grading')) {
            $accordion = false;
        }
    }
    //FIN
    $params = array(
        array(
            //BRICE 'accordionBlocks' => $PAGE->theme->settings->accordionBlocks,
            'accordionBlocks' => $accordion, //BRICE
