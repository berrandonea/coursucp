...
class theme_archaius_core_renderer extends core_renderer {
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        if (isloggedin() && !isguestuser() && 
            $mycourses = enrol_get_my_courses(NULL, 'visible DESC, fullname ASC, idnumber ASC')) {
 
            $branchlabel = get_string('mycourses') ;
            $branchurl   = new moodle_url("$CFG->wwwroot/mescours.php"); //null; //new moodle_url('/my/index.php'); //BRICE course remplacé par my
            $branchtitle = $branchlabel;
            $branchsort  = 8000 ;
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 
            foreach ($mycourses as $mycourse) {
                $branch->add($mycourse->fullname, new moodle_url(
                    '/course/view.php', 
