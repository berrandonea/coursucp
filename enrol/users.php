...
if (!has_capability('moodle/course:viewhiddenuserfields', $context)) {
	...
	if (isset($hiddenfields['idnumber'])) {
        unset($fields['Numéro d\'étudiant']);
    }
...
foreach ($users as $userid=>&$user) {
    ...
    //$user['idnumber'] = $OUTPUT->render($user['idnumber']);
...
$action = "$CFG->wwwroot/report/exportlist/index.php?id=$COURSE->id";
echo '<form enctype="multipart/form-data" action="'.$action.'" method="post">
        <p style="text-align: center;"><input type="submit" value="Exporter la liste des étudiants"/></p>
      </form>';
echo $renderer->render_course_enrolment_users_table($table, $filterform);

//SALMA fichier XML

//Si l'utilisateur est un ingénieur pédagogique ou un gestionnaire change 3 => 1
/*$sql = "SELECT COUNT(id) AS cancompare FROM mdl_role_assignments WHERE userid = $USER->id AND (roleid = 1 OR roleid = 9 OR roleid = 10)";
$cancompare = $DB->get_record_sql($sql)->cancompare;

if ($cancompare > 0) {
    echo '<br><a href="../local/comparaison/comparaison.php?id='.$COURSE->id.'">Comparaison avec un fichier XML</a><br>';
}*/
