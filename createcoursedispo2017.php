...
if (filesize($filename) > 0) {
    ...
    foreach($queryResult as $result){
        ...
        foreach($querycours as $cours){
            ...
            $codecourselp = $idvetyear."-".$codecourselpseul;
            //echo "$codecourselp<br>";
            $course = $DB->get_record('course', array('shortname' => $codecourselp));
            $oldcourse = $DB->get_record('course', array('shortname' => $idvet."-".$codecourselpseul));

            if(!$course && !$oldcourse) {
                // LE SCRIPT JS AFFICHE (DANS LES CAS OU DES COURS SONT TROUVES), LE BLOCK P DE LA VET QUI EST CACHE PAR DEFAUT 
                echo '<script language="javascript" type="text/javascript">document.getElementById("monbeaup'.$idvetyear.'").style.display = "block";</script>' ;
                ...
                if ($showvet == 0) {
                    $showvet = 1;
                    echo "<p id='monbeaup".$idvetyear."' style='font-weight:bold;padding:5px;color:white;background-color : #780D68'>(".$idvetyear.") ".$nomvet."</p>";
                    echo "<ul>";
                }

                echo "<li class='dejacree'><a onmouseover=\"FoToolTip.show(this,'Créer ce cours')\" href=\"$CFG->wwwroot/course/edit.php?category=4&cat=".
                        $idvet."&nomvet=$nomvet&codecours=".$idvetyear."-".$cours->getAttribute('element_pedagogique').
                        "&titrecours=".$cours->getAttribute('libelle_long_element_pedagogique')."".$elpeda."&returnto=category\">".
                        $cours->getAttribute('libelle_long_element_pedagogique')."".$elpeda."</a><br/><i>".$idvetyear."-".
