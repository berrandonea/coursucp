...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    foreach (useredit_get_required_name_fields() as $fullname) {
		...
		{
    	//SALMA $mform->addElement('text', $fullname,  get_string($fullname),  'maxlength="100" size="30"');
        $mform->addElement('text', $fullname,  get_string($fullname), array('size'=>'30', 'maxlength'=>'100', 'readonly'=>'true', 'disabled'=>'true'));
        //$mform->addRule($fullname, $strrequired, 'required', null, 'client');    
        $mform->setType($fullname, PARAM_NOTAGS);
		}
    }
    ...
    if ($user->id > 0 and !empty($CFG->emailchangeconfirmation) and !empty($user->preference_newemail)) {
    	...
    	$mform->addElement('text', 'email', get_string('email'),  array('size'=>'30', 'maxlength'=>'100', 'readonly'=>'true', 'disabled'=>'true'));
        //$mform->addElement('static', 'email', get_string('email'), 'maxlength="100" size="30"');
        //$mform->addRule('email', $strrequired, 'required', null, 'client');
        $mform->setType('email', PARAM_RAW_TRIMMED);
		}
