...
class group_non_members_selector extends groups_user_selector_base {
     ...
     */
    public function output_user($user) {
        return parent::output_user($user) /*BRICE . ' (' . $user->numgroups . ')' */;
    }
    ...
    public function find_users($search
        ...
        //BRICE
        if ($spid > 0) {
            //Groupement associé à la situation problème
            $sql = "SELECT id FROM mdl_groupings WHERE spid = $spid";
            $spgrouping = $DB->get_record_sql($sql);
            
            //Listes des étudiants et des enseignants du cours
            $req = 'SELECT DISTINCT mdl_user.id, lastname, firstname, username, department
                ...
                AND mdl_course.id='. $this->courseid . ' ORDER BY lastname, firstname LIMIT 0 , 30';
            
            $reqTuteur = 'SELECT DISTINCT u.id, u.lastname, u.firstname, u.username, u.department
                FROM mdl_user u, mdl_role_assignments ra, mdl_context ct 
                WHERE u.id = ra.userid                
                AND (ra.roleid = 4 OR ra.roleid = 3)
                AND ra.contextid = ct.id
                AND ct.contextlevel = 50
                AND ct.instanceid = '. $this->courseid . ' ORDER BY u.lastname, u.firstname LIMIT 0 , 30';
            
            $courseTuteurmembers= $DB->get_recordset_sql($reqTuteur);
            $coursemembers= $DB->get_recordset_sql($req);
            $potentialmembers = array();
            $potentialmembers['Étudiant']=array();
            //$potentialmembers['Tuteur']=array();
            ...
            {                               
                $sql ="SELECT COUNT(u.id) as nbuser,u.id,u.firstname,u.lastname,u.email,CONCAT(u.firstname,u.lastname)as fullname  "
                        ...
                        . "AND gg.groupingid = $spgrouping->id AND u.id = $coursemember->id";
             
                $potentialusermembers = new stdClass();               
                
                $user = $DB->get_record_sql($sql);
                if(($user->nbuser) == 0) //Si l'étudiant n'est pas déjà dans l'un des groupes du groupement associé à la situation problème, alors on le met dans la liste
                {                    
                    $potentialusermembers->id = $user->id ;
                    $potentialusermembers->firstname = $user->firstname ;
                    $potentialusermembers->lastname = $user->lastname ;
                    $potentialusermembers->email = $user->email ;
                    $potentialusermembers->fullname = $user->fullname ;
                    $potentialusermembers->fullname = $user->fullname ;
                }
                
                if(!empty($potentialusermembers->id)){
                    $potentialmembers['Étudiant'][$potentialusermembers->id] =$potentialusermembers;
                }
                
            }
           /* On ne propose pas de mettre des enseignants dans les groupes
           unset($potentialusermembers);
            
           foreach($courseTuteurmembers as $courseTuteurmember)
            {
               
                $sql ="SELECT COUNT(u.id) as nbuser,u.id,u.firstname,u.lastname,u.email,CONCAT(u.firstname,u.lastname)as fullname  "
                        . "FROM mdl_user u,mdl_groups_members gm,mdl_sp_groups spg "
                        . "WHERE u.id=gm.userid AND gm.groupid=spg.groupid "
                        . "AND spg.spid=$spid AND u.id=$courseTuteurmember->id";
                
                $potentialusermembers = new stdClass();
                
                
                $user = $DB->get_record_sql($sql);
                if(($user->nbuser) == 0) //idem pour les enseignants
                {                    
                    $potentialusermembers->id = $user->id ;
                    $potentialusermembers->firstname = $user->firstname ;
                    $potentialusermembers->lastname = $user->lastname ;
                ...
                if(!empty($potentialusermembers->id)){
                    $potentialmembers['Tuteur'][$potentialusermembers->id] =$potentialusermembers;
                }
